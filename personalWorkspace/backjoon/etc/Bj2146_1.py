...
visit2 = [[0 for i in range(n)] for _ in range(n)]
dr = [0, 0, -1, 1]
dc = [-1, 1, 0, 0]
...
def BFS(row_index, col_index, island_index):
    ...
    while q:
        popped = q.popleft()
        visit[popped[0]][popped[1]] = island_index
        # 4방향 검색
        for i in range(4):
...
def BFS2(row_index, col_index, island_index):
    ...
    while q:
        ...
        for i in range(4):
            ...
            next_c = popped[1] + dc[i]
            if next_r >= 0 and next_r < n and next_c >= 0 and next_c < n:
                if visit[next_r][next_c] == 0 and (visit2[next_r][next_c] == 0 or visit2[next_r][next_c] > popped[2]+1):
                    visit2[next_r][next_c] = popped[2]+1
