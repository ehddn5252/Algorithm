...
break_wall: bool = False
d_col = [-1,1,0,0]
d_row = [0,0,-1,1]
visit_map = [[False for _ in range(col_num)] for _ in range(row_num) ]
answer_list=[]
answer=9999999999
def backtracking(row,col, break_flag:bool, visit_map, day):
    global answer
    if day>answer:
        return
    ...
    for i in range(4):
        ...
        if 0 <= now_row and now_row<row_num and 0 <= now_col and now_col < col_num:
            if maze_map[now_row][now_col]=="1" and break_flag==False and visit_map[now_row][now_col]==False:
                ...
                backtracking(now_row, now_col, break_flag, visit_map, day+1)
...
backtracking(row=0, col=0, break_flag=False, visit_map=visit_map, day=1)
