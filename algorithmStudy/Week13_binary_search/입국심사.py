...
def solution(n, times):
    ...
    while start<=end:
        mid = (start+end)//2
        comparePeople=0
        for simsadaeTime in times:
            # 4. mid(시간의 중간값) // 심사대에서 걸리는 시간 하면 그 심사대에서 몇 명을 처리했는 지 알 수 있다.
            comparePeople+=mid//simsadaeTime
            # 5. 만약 심사대에서 처리한 수가 n보다 크거나 같다면 정답에 저장하고 end=mid-1로 설정 (bisearch)
            if comparePeople>=n:
                answer=mid
                end=mid-1   
                break
        # 6. 만약 심사대에서 처리한 수가 n보다 작다면 start=mid+1로 두고 다시 while문을 돈다.
        if comparePeople<n:
            start=mid+1
    return answer
